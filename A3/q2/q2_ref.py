...
def softmax(arr: np.ndarray):       # Implements softmax activation function
    ...
    for row in arr:
        for elem in row:
            arr_dummy = []
            arr_dummy.append(1/np.sum(np.exp(arr-elem)))
        new_arr.append(arr_dummy)
    # print(new_arr)
    return np.array(new_arr)

def err_softmax(arr: np.ndarray, real_val: np.ndarray = np.array(0)):       # Softmax error function
    return real_val - arr
...
class NeuralNetwork:
    ...
    def back_propagation(self, Y):
        ...
        self.delta = [err_softmax(self.a[-1], Y)]
...
Y_train = label_encoder.transform(np.expand_dims(Y_train, axis = -1))
# X_test, Y_test = extract_data(args.test_data)
X_test, Y_test = get_data('../data/q2/x_test.npy', '../data/q2/y_test.npy')
# Y_test = label_encoder.transform(np.expand_dims(Y_test, axis = -1))
nn = NeuralNetwork(X_train, Y_train, [100], sigmoid, sigmoid_derivative, 0.1, 1e-20, 32, 100, False, True)
nn.train()
